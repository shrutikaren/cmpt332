/* pipe_driver.c - Virtual FIFO Device Driver */
...
static int device_open(struct inode *inode, struct file *file)
{
    ...
    struct fifo_buffer *fifo = &fifos[fifo_index];
...
{
    int minor = iminor(file_inode(file));  // Get minor number
    int fifo_index = minor / 2;            // Determine FIFO index
    struct fifo_buffer *fifo = &fifos[fifo_index];
    ssize_t ret = 0;
    ...
    size_t bytes_to_read = min(count, (size_t)fifo->count);
    size_t first_chunk = min(bytes_to_read, (size_t)(BUFFER_SIZE - fifo->read_pos));
...
static ssize_t device_write(struct file *file, const char __user *buf, size_t count, loff_t *offset)
{
    ...
    size_t space_available = BUFFER_SIZE - fifo->count;
    size_t bytes_to_write = min(count, space_available);
    size_t first_chunk = min(bytes_to_write, (size_t)(BUFFER_SIZE - fifo->write_pos));
