 ...
 * rttitc.h
 * 
 * Header file for inter-thread communication module.
 * 
 ***************************************************************************
 * 
...
#define ITCTCBINIT( TCB ) {                 \
  SetWaitingFirst(TCB, TNULL);              \
  SetWaitingLast(TCB, TNULL);               \
  SetWaitingNext(TCB, TNULL);               \
  SetWaitingPrev(TCB, TNULL);               \
  SetPeer(TCB, TNULL);                      \
}

#define SETUPITC(TCB) {SetPeer(TCB, TNULL); SetRemoteItc(TCB, 0);}

void ItcThreadCleanup_(TCB *);
