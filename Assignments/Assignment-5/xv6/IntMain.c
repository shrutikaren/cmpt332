...
int main(){
	...
	for (int i = 0; i < child_proc; i++){
		...
		/* If it is 0, we have  child process */
			...
			int sleeping = rand() % 1000;
			int square_limit = 100; /* Could be any number */
			int square_count = rand() % square_limit;
			...
			printf("Child process obtained successfully with the 
			Pid %d.\n", getpid());
			exit(0);
		}
	}
	...
	printf("Our testing is done and we reached the end of our main() 
	function.\n");
}
