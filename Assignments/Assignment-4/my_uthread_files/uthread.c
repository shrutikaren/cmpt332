...
thread_schedule(void)
{
  ...
  printf("i got into the function\n");
  ...
  for(int i = 0; i < MAX_THREAD; i++){
    ...
    if(t >= all_thread + MAX_THREAD){
      	printf("i am inside the first if loop\n");
	t = all_thread;
    }
    if(t->state == RUNNABLE) {
	printf("i am inside the second if loop\n");
      next_thread = t;
      break;
    }
    t = t + 1;
  }
  printf("i finished the for loop");
  ...
  if (current_thread != next_thread) {         /* switch threads?  */
    printf("got into switching of threads if statement \n");
